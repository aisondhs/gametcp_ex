...
func (c *Conn) AsyncReadPacket(timeout time.Duration) (protocol.Packet, error) {
	tempP := protocol.Packet{}
	if c.IsClosed() {
		return tempP, ErrConnClosing
	}
...
func (c *Conn) AsyncWritePacket(p protocol.Packet, timeout time.Duration) error {
	if c.IsClosed() {
...
func (c *Conn) readLoop() {
	...
	for {
		...
		p, err := protocol.ReadPacketCopy(c.conn)
		if err != nil {
			return
		}
